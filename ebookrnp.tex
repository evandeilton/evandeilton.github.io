...
\else % if luatex or xelatex
  ...
  \defaultfontfeatures{Ligatures=TeX,Scale=MatchLowercase}
    \setmonofont[Mapping=tex-ansi,Scale=0.7]{Source Sans Pro}
\fi
...
\section{Como não utilizar
estatística?}\label{como-nao-utilizar-estatistica}

Aqui listamos também nove pontos de atenção para evitar mal uso da
estatística.

\begin{itemize}
\item
  \textbf{Não minta com estatística:} Alguns pesquisadores podem ser
  tentados a maquiar algum dados para seu benefício ou de outros. É
  sempre bom ter em mente que sua reputação e carreira podem estar em
  jogo ao apresentar falsos resultados. Sugerimos aqui uma leitura extra
  do livro do
  \href{https://www.amazon.com.br/Como-Mentir-Estat\%C3\%ADstica-Darrell-Huff/dp/858057952X}{Darrell
  Huff}, pois para não mentir é importante saber como se mente com
  estatística.
\item
  \textbf{Resista aos mau intencionados:} Se alguém te pediu pra fazer
  algo estatisticamente ilegal ou aplicar uma técnica inadequada, seja
  resistente e questione. Nem sempre o problema é simples, então é
  sempre bom ter uma compreensão da situação como um todo. Aprenda a
  dizer não para evitar problemas futuros.
\end{itemize}
...
\begin{quote}
\emph{Big Data is the Information asset characterized by such a High
Volume, Velocity and Variety to require specific Technology and
Analytical Methods for its transformation into Value.}
\end{quote}

Deixamos ao leitor interessado consultar \citep{de2016formal} para mais
definições sobre \emph{Big Data}.
...
\tightlist
\item
  \textbf{Volume Alto} - O termos alto pode ser relativo e depende do
  tipo de computador que está suportando a análise. Nós entendemos como
  volume alto qualquer base de dados que não pode ser processada por um
  sistema de tabulação como \emph{LibreOffice calc} e seu concorrente da
  \emph{Microsoft};
\item
  \textbf{Velocidade} - Grandes bases exigem processamento rápido e
  algorítimos rápidos muitas vezes são mais relevantes que um bom
  \emph{Hardware};
\item
  \textbf{Variedade} - Bancos de dados de fontes diversas podem ser
  relacionados para obter conhecimento.
\end{itemize}

Estes termos chave estão direcionados diretamente com tecnologia e
métodos estatísticos para transformar dados o obter valor ou seja,
conhecimento. Aqui percebemos que \emph{Data Science}, \emph{Big Data}
possuem muito em comum. Ambas utilizam dados como combustível visando
descobrir novos conhecimentos. Ou seja, tudo isso precisa de estatística
para fazer sentido.

Com base nos conceitos até aqui, poderíamos sugerir nossa própria
definição de \emph{Data Science}.

\begin{quote}
``Data Science é uma ciência ampla que une métodos científicos como a
estatística, processos e sistemas tecnológicos buscando, através da
análise de dados, sejam eles simples ou \emph{Big Data} estruturados ou
não, obter conhecimento acerca de fenômenos e processos variados''.
\end{quote}

\section{Conceitos e definições}\label{conceitos-e-definicoes}

A partir de agora iniciamos os estudos de estatística e para melhor
compreender alguns exemplos, vamos trabalhar com o conjunto de dados da
IES do Censo da Educação Superior no Brasil de 2017.
...
Com apoio da base de IES, cuja amostra foi mostrada na tabela
\ref{tab:tab01} vamos exercitar os conceitos vistos até agora.
...
\iffalse{} {Solução. } \fi{}Neste caso, a população se confunde com um
censo, pois a base das IES faz parte do levantamento censitário de 2017
e contempla, a princípio todas as IES do Brasil
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-9}{}{\label{exr:unnamed-chunk-9} }O
que poderia ser uma amostra da base de IES?
\EndKnitrBlock{exercise}

\BeginKnitrBlock{solution}
\iffalse{} {Solução. } \fi{}Uma amostra poderia ser todas as IES grandes
com mais de 2000 técnicos.
\EndKnitrBlock{solution}
...
Como podemos notar, a figura \ref{fig:graf01} é muito intuitiva para
representar visualmente a distribuição de frequências das classes de uma
variável categórica. Combinado com as cores de cada fatia, fica claro e
objetivo a parcela de cada categoria para explicar o todo que por sua
vez representa 100\%.

\subsubsection{Barras}\label{barras}

Gráficos de barras também são intuitivos e geralmente são preferíveis em
relação aos gráficos de setores. Isso ocorre porque o olho humano é mais
sensível a linhas do que círculos e formas em 3D e prefere analisar
figuras mais limpas. Para expandir seus conhecimentos sobre análise
visual, sugerimos a \citep{tufte2014visual}.
...
\BeginKnitrBlock{solution}
\iffalse{} {Solução. } \fi{}Para dados categóricos, os gráficos mais
utilizados são de barras e setores. Vamos fazer um gráfico de barras com
ggplot2.
\EndKnitrBlock{solution}
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-42-1} 

}

\end{figure}

As categorias privada com e sem fins lucrativos representam 70,1\% dos
dados, sendo as mais representativas. Públicas municipais são minoria
(1,4\%) e especial apenas (0.2\%). Acho que políticas públicas para
aumentar a participação de cursos especiais devem ser melhoradas.

\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-43}{}{\label{exr:unnamed-chunk-43}
}Olhando para a variável \texttt{DESC\_TP\_GRAU\_ACADEMICO} responda
quais as categorias de maior e menor ocorrências de cursos. Desconsidere
a categoria missing.
\EndKnitrBlock{exercise}

\BeginKnitrBlock{solution}
\iffalse{} {Solução. } \fi{}Vamos gerar uma tabela de frequências
simples para avaliar as categorias e depois eliminar os dados da
categoria missing.
\EndKnitrBlock{solution}
...
\NormalTok{GrauAcad <-}\StringTok{ }\NormalTok{base_curso }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{transmute}\NormalTok{(}\DataTypeTok{GrauAcad =}\NormalTok{ DESC_TP_GRAU_ACADEMICO) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{filter}\NormalTok{(GrauAcad }\OperatorTok{%in%}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"1. Bacharelado"}\NormalTok{, }\StringTok{"2. Licenciatura"}\NormalTok{, }\StringTok{"3. Tecnológico"))}
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-45-1} 
...
\StringTok{    }\KeywordTok{summarise}\NormalTok{(}\StringTok{"Média"}\NormalTok{   =}\StringTok{ }\KeywordTok{mean}\NormalTok{(.[[i]], }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{),}
              \StringTok{"Mediana"}\NormalTok{ =}\StringTok{ }\KeywordTok{median}\NormalTok{(.[[i]], }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{))}
\NormalTok{\}) }\OperatorTok{%>%}\KeywordTok{t}\NormalTok{()}

\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(tb, }\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata, }\DataTypeTok{digits =} \DecValTok{3}\NormalTok{,}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{tb <-}\StringTok{ }\NormalTok{rnp}\OperatorTok{::}\KeywordTok{rnp_summary}\NormalTok{(base_ies}\OperatorTok{$}\NormalTok{VL_DESPESA_PESSOAL_DOCENTE)[}\OperatorTok{-}\KeywordTok{c}\NormalTok{(}\DecValTok{1}\OperatorTok{:}\DecValTok{3}\NormalTok{)]}
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-83-1} 
...
O resultado é um gráfico elegante e que indica precisamente a forma da
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-85-1} 

}

\caption{Densidade de idade vs escolaridade por sexo}\label{fig:unnamed-chunk-85}
\end{figure}

\begin{itemize}
\tightlist
\item
  \textbf{Gráfico de colunas agrupadas}: é possível assim, como nos dois
  últimos exemplos, gerar gráficos e coluna agrupadas e fazer a quebra
...
\caption{Colunas de idade vs escolaridade por sexo}\label{fig:unnamed-chunk-86}
\end{figure}

\begin{itemize}
\tightlist
\item
  \textbf{Histogramas agrupados}: Histogramas também são agrupáveis
  segundo as categorias da variável.
\end{itemize}
