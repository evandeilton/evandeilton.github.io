...
\NormalTok{dicionario <-}\StringTok{ "Dados/INEP/ANEXOS/ANEXO I - Dicionario de Dados e Tabelas Auxiliares/Dicionario_de_Dados.xlsx"}
...
\StringTok{  }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata,}
               \DataTypeTok{caption =} \StringTok{"ex: Frequências para categoria administrativa (cursos)"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# As categorias diferentes de missing são: 1. Bacharelado, 2. Licenciatura e 3. Tecnológico}
\CommentTok{# missing representa 0.09% dos dados.}
...
\StringTok{  dplyr::filter(GrauAcad %in% grau)}
\StringTok{tb <- rnp::rnp_freq(GrauAcad$GrauAcad, digits = 3)}
\StringTok{p <- ggplot2::ggplot(tb, aes(classe, fr, fill = classe))}
\StringTok{p + ggplot2::geom_bar(width = 1, size = 0.6, color = "}\NormalTok{white}\StringTok{", }
\StringTok{                      stat = "}\NormalTok{identity}\StringTok{", show.legend = FALSE) +}
\StringTok{  ggplot2::geom_text(aes(label = paste0(round(100*fr, 1), "}\NormalTok{%}\StringTok{")),}
\StringTok{                     position = position_stack(vjust = 1), size=3) +}
\StringTok{  ggplot2::labs(x = NULL, y = NULL, fill = NULL, title = "") +}
\StringTok{  ggplot2::theme_gray() + coord_flip() +}
...
\StringTok{  knitr::kable(caption = "}\NormalTok{Média anual para dados de IES}\StringTok{", format = tb_formata) %>% }
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{summarise}\NormalTok{(}\StringTok{"Receita própria"  = mean(ReceitaPropria),}
\StringTok{                   "}\NormalTok{Receita própria ponderada}\StringTok{" = weighted.mean(x = ReceitaPropria,}
\StringTok{                                                               w = TotalTecnicos)) %>%}
\StringTok{  knitr::kable(format = tb_formata,}
\StringTok{               caption = "}\NormalTok{Média receita própria ponderada pelo total de técnicos}\StringTok{") %>% }
\StringTok{  kableExtra::kable_styling(latex_options = "}\NormalTok{hold_position}\StringTok{")}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{summarise}\NormalTok{(}\StringTok{`}\DataTypeTok{Média receita}\StringTok{`}\NormalTok{ =}\StringTok{ }\KeywordTok{mean}\NormalTok{(VL_RECEITA_PROPRIA),}
                   ...
                   \StringTok{`}\DataTypeTok{CV receita}\StringTok{`}\NormalTok{ =}\StringTok{ }\KeywordTok{sd}\NormalTok{(VL_RECEITA_PROPRIA) }\OperatorTok{/}\StringTok{ }\KeywordTok{mean}\NormalTok{(VL_RECEITA_PROPRIA),}
