...
%
\documentclass[11pt,]{style/krantz}
\usepackage{lmodern}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Informe o ano dos dados e o local completo de onde deseja salvar os dados}
\CommentTok{# conforme exemplos abaixo. Recomendamos criar uma pasta para salvar os}
\CommentTok{# dados e torná-los mais acessíveis nas aulas.}
...
\NormalTok{dicionario <-}\StringTok{ "Dados/INEP/ANEXOS/ANEXO I - Dicionario de Dados e Tabelas}
\StringTok{Auxiliares/Dicionario_de_Dados.xlsx"}
...
\CommentTok{# Leitura da base de dados  }
...
\CommentTok{# Aplicando classes aos dados}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Carregando pacote rnp}
\KeywordTok{require}\NormalTok{(rnp, }\DataTypeTok{quietly =} \OtherTok{TRUE}\NormalTok{)}

\CommentTok{# Dados IES}
\NormalTok{base_ies <-}\StringTok{ }\NormalTok{rnp}\OperatorTok{::}\NormalTok{dm_ies }
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Sigla =}\NormalTok{ SG_IES,}
                \DataTypeTok{TotalTecnicos =}\NormalTok{ QT_TEC_TOTAL, }
                \DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA,}
                \DataTypeTok{DespesaPesquisa =}\NormalTok{ VL_DESPESA_PESQUISA) }\OperatorTok{%>%}\StringTok{ }
...
\StringTok{  }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{digits =} \DecValTok{2}\NormalTok{, }\DataTypeTok{align =} \StringTok{"llrrr"}\NormalTok{, }
             ...
             \DataTypeTok{caption =} \StringTok{"Dez primeiras observações base de IES"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{paste}\NormalTok{(}\StringTok{"A base tem"}\NormalTok{, }
\NormalTok{      base_ies }\OperatorTok{%>%}\StringTok{ }\KeywordTok{transmute}\NormalTok{(}\DataTypeTok{TotalTecnicos =}\NormalTok{ QT_TEC_TOTAL) }\OperatorTok{%>%}\StringTok{ }
\StringTok{        }\KeywordTok{filter}\NormalTok{(TotalTecnicos }\OperatorTok{>}\StringTok{ }\DecValTok{2000}\NormalTok{) }\OperatorTok{%>%}
\StringTok{        }\KeywordTok{nrow}\NormalTok{(), }\StringTok{"IES com mais de 2000 técnicos"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{base_docentes <-}\StringTok{ }\NormalTok{rnp}\OperatorTok{::}\NormalTok{dm_docente}
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{faixaIdade =} \KeywordTok{if_else}\NormalTok{(NU_IDADE }\OperatorTok{<=}\StringTok{ }\DecValTok{30}\NormalTok{, }\StringTok{"01.Até 30 anos"}\NormalTok{,}
                             ...
                             \StringTok{"05.Acima de 60 anos"}\NormalTok{))))) }\OperatorTok{%>%}
...
\addlinespace
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_DEFICIENCIA\_AUDITIVA & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_DEFICIENCIA\_BAIXA\_VISAO & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_DEFICIENCIA\_CEGUEIRA & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_DEFICIENCIA\_FISICA & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_DEFICIENCIA\_INTELECTUAL & character\\
\addlinespace
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_DEFICIENCIA\_MULTIPLA & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_DEFICIENCIA\_SURDEZ & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_DEFICIENCIA\_SURDOCEGUEIRA & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_EXERCICIO\_DATA\_REFERENCIA & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_SUBSTITUTO & character\\
\addlinespace
data.frame & 392036 linhas e 42 colunas & DESC\_IN\_VISITANTE & character\\
data.frame & 392036 linhas e 42 colunas & catAdm & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_TP\_COR\_RACA & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_TP\_DEFICIENCIA & character\\
data.frame & 392036 linhas e 42 colunas & escolaridade & character\\
\addlinespace
data.frame & 392036 linhas e 42 colunas & DESC\_TP\_NACIONALIDADE & character\\
data.frame & 392036 linhas e 42 colunas & DESC\_TP\_ORGANIZACAO\_ACADEMICA & character\\
...
\addlinespace
data.frame & 392036 linhas e 42 colunas & DESC\_TP\_VISITANTE\_IFES\_VINCULO & character\\
...
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata,}
             \DataTypeTok{caption =} \StringTok{"Frequências simples escolaridade do docente"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
...
{\centering \includegraphics[width=1\linewidth]{../Imagens/ebook_tabela_contingencia} 
...
\NormalTok{rnp}\OperatorTok{::}\KeywordTok{rnp_2freq}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ base_docentes}\OperatorTok{$}\NormalTok{escolaridade,}
               ...
               \DataTypeTok{digits =} \DecValTok{3}\NormalTok{, }\DataTypeTok{percents =} \OtherTok{FALSE}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata,}
             \DataTypeTok{caption =} \StringTok{"Frequências cruzadas da escolaridade por sexo"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\caption{\label{tab:tab05}Frequências cruzadas da escolaridade por sexo}
\centering
\begin{tabular}{llrrr}
\toprule
...
\BeginKnitrBlock{solution}
\iffalse{} {Solução. } \fi{}(a) Na tabela cruzada, doutores ficam na quinta linha, logo fixamos a linha \texttt{i\ =\ 5} e fazemos a soma das colunas de 1 a 2 para sexo.
...
\tightlist
\item
  neste caso temos \texttt{j=("1.Feminino","02.Masculino")}, logo fazemos a soma das linhas fixando cada coluna.
\end{enumerate}
...
\StringTok{  }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{theme}\NormalTok{(}\DataTypeTok{axis.line =} \KeywordTok{element_blank}\NormalTok{(),}
                 ...
                 \DataTypeTok{legend.text =} \KeywordTok{element_text}\NormalTok{(}\StringTok{"Classe"}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rnp}\OperatorTok{::}\KeywordTok{rnp_freq}\NormalTok{(base_curso}\OperatorTok{$}\NormalTok{DESC_TP_CATEGORIA_ADMINISTRATIVA, }\DataTypeTok{digits =} \DecValTok{3}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata,}
               \DataTypeTok{caption =} \StringTok{"Frequências categoria administrativa"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\BeginKnitrBlock{exercise}
\protect\hypertarget{exr:unnamed-chunk-38}{}{\label{exr:unnamed-chunk-38} }Represente graficamente a variável \texttt{DESC\_TP\_CATEGORIA\_ADMINISTRATIVA} e interprete quais as categorias de maior e menor influência.
\EndKnitrBlock{exercise}

\BeginKnitrBlock{solution}
\iffalse{} {Solução. } \fi{}Para dados categóricos, os gráficos mais utilizados são de barras e setores. Vamos fazer um gráfico de barras com ggplot2.
\EndKnitrBlock{solution}

\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Primeiro fazemos as frequências, depois o gráfico}
\NormalTok{tb <-}\StringTok{ }\NormalTok{rnp}\OperatorTok{::}\KeywordTok{rnp_freq}\NormalTok{(base_curso}\OperatorTok{$}\NormalTok{DESC_TP_CATEGORIA_ADMINISTRATIVA)}

\NormalTok{p <-}\StringTok{ }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{ggplot}\NormalTok{(tb, }\KeywordTok{aes}\NormalTok{(classe, fr, }\DataTypeTok{fill =}\NormalTok{ classe))}
\NormalTok{p }\OperatorTok{+}\StringTok{ }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{width =} \DecValTok{1}\NormalTok{, }\DataTypeTok{size =} \FloatTok{0.6}\NormalTok{, }\DataTypeTok{color =} \StringTok{"white"}\NormalTok{, }
                      \DataTypeTok{stat =} \StringTok{"identity"}\NormalTok{, }\DataTypeTok{show.legend =} \OtherTok{FALSE}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{geom_text}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{label =} \KeywordTok{paste0}\NormalTok{(}\KeywordTok{round}\NormalTok{(}\DecValTok{100}\OperatorTok{*}\NormalTok{fr, }\DecValTok{1}\NormalTok{), }\StringTok{"%"}\NormalTok{)),}
                     \DataTypeTok{position =} \KeywordTok{position_stack}\NormalTok{(}\DataTypeTok{vjust =} \DecValTok{1}\NormalTok{), }\DataTypeTok{size=}\DecValTok{3}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{labs}\NormalTok{(}\DataTypeTok{x =} \OtherTok{NULL}\NormalTok{, }\DataTypeTok{y =} \OtherTok{NULL}\NormalTok{, }\DataTypeTok{fill =} \OtherTok{NULL}\NormalTok{, }\DataTypeTok{title =} \StringTok{""}\NormalTok{) }\OperatorTok{+}
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-40-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# As categorias diferentes de missing são: }
\CommentTok{# 1. Bacharelado, 2. Licenciatura e 3. Tecnológico}
\CommentTok{# missing representam 0.09% dos dados.}

\NormalTok{grau <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"1. Bacharelado"}\NormalTok{, }\StringTok{"2. Licenciatura"}\NormalTok{, }\StringTok{"3. Tecnológico")}
\StringTok{GrauAcad <- base_curso %>% }
\StringTok{  dplyr::transmute(GrauAcad = DESC_TP_GRAU_ACADEMICO) %>% }
\StringTok{  dplyr::filter(GrauAcad %in% c("}\FloatTok{1.}\NormalTok{ Bacharelado}\StringTok{", }
\StringTok{                                "}\FloatTok{2.}\NormalTok{ Licenciatura}\StringTok{", }
\StringTok{                                "}\FloatTok{3.}\NormalTok{ Tecnológico")}\ErrorTok{)}

\NormalTok{tb <-}\StringTok{ }\NormalTok{rnp}\OperatorTok{::}\KeywordTok{rnp_freq}\NormalTok{(GrauAcad}\OperatorTok{$}\NormalTok{GrauAcad, }\DataTypeTok{digits =} \DecValTok{3}\NormalTok{)}
\NormalTok{p <-}\StringTok{ }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{ggplot}\NormalTok{(tb, }\KeywordTok{aes}\NormalTok{(classe, fr, }\DataTypeTok{fill =}\NormalTok{ classe))}
\NormalTok{p }\OperatorTok{+}\StringTok{ }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{width =} \DecValTok{1}\NormalTok{, }\DataTypeTok{size =} \FloatTok{0.6}\NormalTok{, }\DataTypeTok{color =} \StringTok{"white"}\NormalTok{, }
                      \DataTypeTok{stat =} \StringTok{"identity"}\NormalTok{, }\DataTypeTok{show.legend =} \OtherTok{FALSE}\NormalTok{) }\OperatorTok{+}
\StringTok{  }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{geom_text}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{label =} \KeywordTok{paste0}\NormalTok{(}\KeywordTok{round}\NormalTok{(}\DecValTok{100}\OperatorTok{*}\NormalTok{fr, }\DecValTok{1}\NormalTok{), }\StringTok{"%"}\NormalTok{)),}
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-43-1} 
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{Sigla =}\NormalTok{ SG_IES,}
                ...
                \DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA,}
                \DataTypeTok{DespesaPesquisa =}\NormalTok{ VL_DESPESA_PESQUISA) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{summarise}\NormalTok{(}\StringTok{"Total técnicos"}\NormalTok{  =}\StringTok{ }\KeywordTok{mean}\NormalTok{(TotalTecnicos),}
                   \StringTok{"Receita própria" = mean(ReceitaPropria),}
\StringTok{                   "}\NormalTok{Depesa pesquisa}\StringTok{" = mean(DespesaPesquisa)) %>% }
\StringTok{  knitr::kable(caption = "}\NormalTok{Média anual para dados de IES}\StringTok{", }
\StringTok{               format = tb_formata) %>% }
\StringTok{  kableExtra::kable_styling(latex_options = "}\NormalTok{hold_position}\StringTok{")}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{base_ies }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{TotalTecnicos =}\NormalTok{ QT_TEC_TOTAL, }
                \DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{summarise}\NormalTok{(}\StringTok{"Receita"}\NormalTok{  =}\StringTok{ }\KeywordTok{mean}\NormalTok{(ReceitaPropria),}
                   \StringTok{"Receita ponderada"}\NormalTok{ =}\StringTok{ }
\StringTok{                     }\KeywordTok{weighted.mean}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ ReceitaPropria,}
                                   \DataTypeTok{w =}\NormalTok{ TotalTecnicos)) }\OperatorTok{%>%}
\StringTok{  }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{format =}\NormalTok{ tb_formata,}
               \DataTypeTok{caption =} \StringTok{"Média receita própria }
\StringTok{               ponderada pelo total de técnicos"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{variaveis <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"QT_TEC_MEDIO_FEM"}\NormalTok{, }\StringTok{"QT_TEC_MEDIO_MASC"}\NormalTok{, }
               \StringTok{"QT_TEC_SUPERIOR_FEM"}\NormalTok{, }\StringTok{"QT_TEC_SUPERIOR_MASC"}\NormalTok{)}
\NormalTok{tb <-}\StringTok{ }\KeywordTok{sapply}\NormalTok{(variaveis, }\ControlFlowTok{function}\NormalTok{(i) \{}
\NormalTok{  base_ies }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{select}\NormalTok{(i) }\OperatorTok{%>%}\StringTok{ }
\StringTok{    }\KeywordTok{summarise}\NormalTok{(}\StringTok{"Média"}\NormalTok{   =}\StringTok{ }\KeywordTok{round}\NormalTok{(}\KeywordTok{mean}\NormalTok{(.[[i]], }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{), }\DecValTok{2}\NormalTok{),}
              \StringTok{"Mediana"}\NormalTok{ =}\StringTok{ }\KeywordTok{round}\NormalTok{(}\KeywordTok{median}\NormalTok{(.[[i]], }\DataTypeTok{na.rm =} \OtherTok{TRUE}\NormalTok{),}\DecValTok{2}\NormalTok{))}
\NormalTok{  \}) }\OperatorTok{%>%}\KeywordTok{t}\NormalTok{()}

\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(tb, }\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata,}
             \DataTypeTok{caption =} \StringTok{"Média e mediana técnicos (IES)"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{paste}\NormalTok{(}\StringTok{"Média ="}\NormalTok{, }\KeywordTok{round}\NormalTok{(}\KeywordTok{mean}\NormalTok{(base_docentes}\OperatorTok{$}\NormalTok{idade), }\DecValTok{3}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{base_ies }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{summarise}\NormalTok{(}\StringTok{`}\DataTypeTok{Média receita}\StringTok{`}\NormalTok{ =}\StringTok{ }\KeywordTok{mean}\NormalTok{(VL_RECEITA_PROPRIA),}
                   \StringTok{`}\DataTypeTok{Média técnicos}\StringTok{`}\NormalTok{ =}\StringTok{ }\KeywordTok{mean}\NormalTok{(QT_TEC_TOTAL),}
                   \StringTok{`}\DataTypeTok{CV receita}\StringTok{`}\NormalTok{ =}\StringTok{ }\KeywordTok{c}\NormalTok{(}\KeywordTok{sd}\NormalTok{(VL_RECEITA_PROPRIA) }\OperatorTok{/}
\StringTok{                     }\KeywordTok{mean}\NormalTok{(VL_RECEITA_PROPRIA)),}
                   \StringTok{`}\DataTypeTok{CV técnicos}\StringTok{`}\NormalTok{ =}\StringTok{ }\KeywordTok{sd}\NormalTok{(QT_TEC_TOTAL) }\OperatorTok{/}\StringTok{ }\KeywordTok{mean}\NormalTok{(QT_TEC_TOTAL)) }\OperatorTok{%>%}\StringTok{ }
...
\StringTok{    }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{summarise}\NormalTok{(}\DataTypeTok{Q1  =} \KeywordTok{quantile}\NormalTok{(idade, }\DataTypeTok{probs =} \FloatTok{0.25}\NormalTok{),}
                     ...
                     \DataTypeTok{Pct  =}\NormalTok{ Noutliers }\OperatorTok{/}\StringTok{ }\NormalTok{Ntotal) }
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{estatisticas <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\StringTok{"Min"}\NormalTok{,}\StringTok{"Q1"}\NormalTok{,}\StringTok{"Media"}\NormalTok{,}\StringTok{"Mediana"}\NormalTok{,}\StringTok{"Q3"}\NormalTok{,}\StringTok{"Max"}\NormalTok{,}\StringTok{"cv"}\NormalTok{)}
\NormalTok{tb <-}\StringTok{ }\NormalTok{rnp}\OperatorTok{::}\KeywordTok{rnp_summary}\NormalTok{(base_ies}\OperatorTok{$}\NormalTok{VL_DESPESA_PESSOAL_DOCENTE)[estatisticas]}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\KeywordTok{t}\NormalTok{(tb), }\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata, }\DataTypeTok{digits =} \DecValTok{3}\NormalTok{, }
             \DataTypeTok{caption =} \StringTok{"ex: Média e mediana técnicos (IES)"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{base_ies }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{transmute}\NormalTok{(}\DataTypeTok{NomeIES =}\NormalTok{ NO_IES, }\DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{arrange}\NormalTok{(}\KeywordTok{desc}\NormalTok{(ReceitaPropria)) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{head}\NormalTok{(}\DataTypeTok{n =} \DecValTok{10}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata, }\DataTypeTok{digits =} \DecValTok{3}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:unnamed-chunk-78}{}{\label{exm:unnamed-chunk-78} }Ainda para os dados de idade, vamos montar um Box-plot
\EndKnitrBlock{example}
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:unnamed-chunk-79}{}{\label{exm:unnamed-chunk-79} }Para lustrar vamos traçar um gráfico de pontos para a receita próprias das IES pelo total de técnicos na base das IES.
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA,}
                \DataTypeTok{TotalTecnicos =}\NormalTok{ QT_TEC_TOTAL) }\OperatorTok{%>%}\StringTok{ }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{## Categorizando por quartis}
\NormalTok{base_ies <-}\StringTok{ }\NormalTok{base_ies }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{mutate}\NormalTok{(}\DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA,}
                \DataTypeTok{TotalTecnicos =}\NormalTok{ QT_TEC_TOTAL) }
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{## Categorização por ifelse com operadores relacionais}
\NormalTok{fx_receita_r <-}\StringTok{ }\KeywordTok{ifelse}\NormalTok{(base_ies}\OperatorTok{$}\NormalTok{ReceitaPropria }\OperatorTok{<=}\StringTok{ }\DecValTok{1200000}\NormalTok{, }
                       \StringTok{"A.-1200000"}\NormalTok{,}
                \KeywordTok{ifelse}\NormalTok{(base_ies}\OperatorTok{$}\NormalTok{ReceitaPropria }\OperatorTok{<=}\StringTok{ }\DecValTok{5000000}\NormalTok{, }
                       \StringTok{"B.1200000 a 5000000"}\NormalTok{,}
                \KeywordTok{ifelse}\NormalTok{(base_ies}\OperatorTok{$}\NormalTok{ReceitaPropria }\OperatorTok{<=}\DecValTok{35000000}\NormalTok{, }
                       \StringTok{"C.5000000 a 35000000"}\NormalTok{,}\StringTok{"D.35000000+"}\NormalTok{)))}
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-84-1} 
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-86-1} 

}

\caption{Densidade de idade vs escolaridade por sexo}\label{fig:unnamed-chunk-86}
\end{figure}

\begin{itemize}
\tightlist
\item
  \textbf{Gráfico de colunas agrupadas}: é possível assim, como nos dois últimos exemplos, gerar gráficos e coluna agrupadas e fazer a quebra adicionando uma terceira variável.
\end{itemize}
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-87-1} 

}

\caption{Colunas de idade vs escolaridade por sexo}\label{fig:unnamed-chunk-87}
\end{figure}
...
{\centering \includegraphics{ebookrnp_files/figure-latex/unnamed-chunk-88-1} 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Sem utilizar a chisq.test()}
\NormalTok{observado <-}\StringTok{ }\KeywordTok{table}\NormalTok{(base_docentes}\OperatorTok{$}\NormalTok{escolaridade, base_docentes}\OperatorTok{$}\NormalTok{sexo)}
\NormalTok{observado}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Com a chisq.test()}
\NormalTok{t1 <-}\StringTok{ }\KeywordTok{chisq.test}\NormalTok{(observado)}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{t1}\OperatorTok{$}\NormalTok{statistic}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{data}\NormalTok{(dm_curso, }\DataTypeTok{package =} \StringTok{"rnp"}\NormalTok{)}
\CommentTok{# Tabela de contingência}
\NormalTok{observado <-}\StringTok{ }\KeywordTok{table}\NormalTok{(dm_curso}\OperatorTok{$}\NormalTok{DESC_IN_MATERIAL_BRAILLE, }
\NormalTok{                   dm_curso}\OperatorTok{$}\NormalTok{DESC_IN_MATERIAL_TATIL)}

\CommentTok{# Com a chisq.test()}
\NormalTok{t1 <-}\StringTok{ }\KeywordTok{chisq.test}\NormalTok{(observado)}

\CommentTok{# Estatística e Range}
\NormalTok{t1}\OperatorTok{$}\NormalTok{statistic}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\KeywordTok{data}\NormalTok{(dm_curso, }\DataTypeTok{package =} \StringTok{"rnp"}\NormalTok{)}
\NormalTok{rnp}\OperatorTok{::}\KeywordTok{rnp_associacao}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ dm_curso}\OperatorTok{$}\NormalTok{DESC_IN_MATERIAL_BRAILLE, }
                    \DataTypeTok{y =}\NormalTok{ dm_curso}\OperatorTok{$}\NormalTok{DESC_IN_MATERIAL_TATIL)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{base_ies }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{transmute}\NormalTok{(}\DataTypeTok{TotalTecnicos =}\NormalTok{ QT_TEC_TOTAL, }
                   \DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA,}
                   \DataTypeTok{DespesaPesquisa =}\NormalTok{ VL_DESPESA_PESQUISA) }\OperatorTok{%>%}\StringTok{ }
...
\BeginKnitrBlock{example}
\protect\hypertarget{exm:unnamed-chunk-106}{}{\label{exm:unnamed-chunk-106} }Determinar e interpretar a correlação entre as duas variáveis \texttt{TotalTecnicos} e \texttt{ReceitaPropria} para as 10 primeiaras observações da base de dados \texttt{dm\_ies}.
\EndKnitrBlock{example}

\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{temp <-}\StringTok{ }\NormalTok{base_ies }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{head}\NormalTok{(}\DataTypeTok{n =} \DecValTok{10}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{transmute}\NormalTok{(}\DataTypeTok{TotalTecnicos =}\NormalTok{ QT_TEC_TOTAL,}
                   \StringTok{`}\DataTypeTok{R(xi)}\StringTok{`}\NormalTok{ =}\StringTok{ }\KeywordTok{rank}\NormalTok{(}\OperatorTok{-}\NormalTok{TotalTecnicos),}
                   \DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA,}
                   \StringTok{`}\DataTypeTok{R(yi)}\StringTok{`}\NormalTok{ =}\StringTok{ }\KeywordTok{rank}\NormalTok{(}\OperatorTok{-}\NormalTok{ReceitaPropria),}
                   \DataTypeTok{di =} \StringTok{`}\DataTypeTok{R(xi)}\StringTok{`}\OperatorTok{-}\StringTok{`}\DataTypeTok{R(yi)}\StringTok{`}\NormalTok{,}
                   \DataTypeTok{di2 =}\NormalTok{ di}\OperatorTok{^}\DecValTok{2}\NormalTok{)}
\CommentTok{# Tabela de exemplo}
\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ temp, }\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata,}
             \DataTypeTok{caption =} \StringTok{"Análise de correlação de Spearman"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculando manualmente}
\DecValTok{1} \OperatorTok{-}\StringTok{ }\NormalTok{(}\DecValTok{6}\OperatorTok{*}\KeywordTok{sum}\NormalTok{(temp}\OperatorTok{$}\NormalTok{di2))}\OperatorTok{/}\NormalTok{(}\KeywordTok{nrow}\NormalTok{(temp)}\OperatorTok{*}\NormalTok{(}\KeywordTok{nrow}\NormalTok{(temp)}\OperatorTok{^}\DecValTok{2-1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# Calculando via R}
\KeywordTok{cor}\NormalTok{(temp}\OperatorTok{$}\NormalTok{TotalTecnicos, temp}\OperatorTok{$}\NormalTok{ReceitaPropria, }\DataTypeTok{method =} \StringTok{"spearman"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\StringTok{  }\NormalTok{dplyr}\OperatorTok{::}\KeywordTok{transmute}\NormalTok{(}\DataTypeTok{TotalTecnicos =}\NormalTok{ QT_TEC_TOTAL,}
                   \DataTypeTok{ReceitaPropria =}\NormalTok{ VL_RECEITA_PROPRIA,}
                   \DataTypeTok{DespesaPesquisa =}\NormalTok{ VL_DESPESA_PESQUISA)}
\NormalTok{rnp}\OperatorTok{::}\KeywordTok{rnp_correlacao}\NormalTok{(temp) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{knitr}\OperatorTok{::}\KeywordTok{kable}\NormalTok{(}\DataTypeTok{booktabs =} \OtherTok{TRUE}\NormalTok{, }\DataTypeTok{format =}\NormalTok{ tb_formata, }\DataTypeTok{row.names =} \OtherTok{FALSE}\NormalTok{,}
             \DataTypeTok{caption =} \StringTok{"Correlação com rnp_correlacao()"}\NormalTok{) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\NormalTok{kableExtra}\OperatorTok{::}\KeywordTok{kable_styling}\NormalTok{(}\DataTypeTok{latex_options =} \StringTok{"hold_position"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
